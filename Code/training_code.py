...
def calculate_overall_f1(prediction_data):
    ...
    fm_classification_report = classification_report(fm_label, fm_pred, output_dict = True)
    fm_cr_df = pd.DataFrame(fm_classification_report).transpose()

    fm_confusion_matrix = confusion_matrix(fm_label, fm_pred)
    fm_cm_df = pd.DataFrame(fm_confusion_matrix)

    saho_classification_report = classification_report(saho_label, saho_pred, output_dict = True)
    saho_cr_df = pd.DataFrame(saho_classification_report).transpose()

    saho_confusion_matrix = confusion_matrix(saho_label, saho_pred)
    saho_cm_df = pd.DataFrame(saho_confusion_matrix)

    sc_classification_report = classification_report(sc_label, sc_pred, output_dict = True)
    sc_cr_df = pd.DataFrame(sc_classification_report)

    sc_confusion_matrix = confusion_matrix(sc_label, sc_pred)
    sc_cm_df = pd.DataFrame(sc_confusion_matrix)
...
if __name__ == '__main__':
    ...
    for loop_index in range(n_rounds):
        for model_name in models:
            ...
            model_save_location = '../20_epochs_large_model/saved_models_2a/' + model_name + '/' + str(loop_index) + '/' 
            model_load_location = None

            epoch_save_location = '../20_epochs_large_model/saved_epoch_2a/' + model_name + '/' + str(loop_index) + '/' 
            epoch_save_name = epoch_save_location + '/epoch_info.tsv'

            result_save_location = '../20_epochs_large_model/saved_data_2a/' + model_name + '/' + str(loop_index) + '/'

            report_result_save_location = '../20_epochs_large_model/saved_report_2a/' + model_name + '/' + str(loop_index)
            ...
            best_prediction_result, best_dev_acc, best_test_acc, best_tb_acc, best_epoch, best_tb_epoch, best_overall_f1_score, best_ind_f1_score, best_ind_precision, best_ind_recall, epoch_data = main(n_epochs, model_name, model_save_flag, model_save_location, model_load_flag, model_load_location, report_result_save_location)
    ...
    os.makedirs('../20_epochs_large_model/validating_statistics/', exist_ok=True)
    all_best_overall_f1_score.to_csv('../20_epochs_large_model/validating_statistics/all_best_overall_f1_score.tsv', sep='\t')
    all_best_dev_acc.to_csv('../20_epochs_large_model/validating_statistics/all_best_dev_acc.tsv', sep='\t')
    all_best_ind_f1_score.to_csv('../20_epochs_large_model/validating_statistics/all_best_ind_f1_score.tsv', sep='\t')
    all_best_ind_precision.to_csv('../20_epochs_large_model/validating_statistics/all_best_ind_precision.tsv', sep='\t')
    all_best_ind_recall.to_csv('../20_epochs_large_model/validating_statistics/all_best_ind_recall.tsv', sep='\t')
    ...
    with open('../20_epochs_large_model/validating_statistics/time.txt', 'w') as file:
